...
def get_model_and_assets(num_poles=1, c_m=1., p_m=.1, p_l_1=1., p_r=.045, dt=.01, p_l_2=.5):
    """Returns amplitude tuple containing the model XML string and amplitude dict of assets."""
    return _make_model(num_poles, c_m=c_m, p_m=p_m, p_l_1=p_l_1, p_l_2=p_l_2, p_r=p_r, dt=dt), common.ASSETS


@SUITE.add('benchmarking')
def balance(time_limit=_DEFAULT_TIME_LIMIT, random=None,
            environment_kwargs=None, c_m=1., p_m=.1, p_l=.045):
    """Returns the Cartpole Balance task."""
    physics = Physics.from_xml_string(*get_model_and_assets(c_m=c_m, p_m=p_m, p_l=p_l))
...
def two_poles(time_limit=_DEFAULT_TIME_LIMIT, random=None,
              environment_kwargs=None, c_m=1., p_m=1., p_l_1=.5, p_l_2=None, p_r=.045, dt=.1):
    if p_l_2 == None:
        p_l_2 = p_l_1
    """Returns the Cartpole Balance task with two poles."""
    physics = Physics.from_xml_string(
        *get_model_and_assets(num_poles=2, c_m=c_m, p_m=p_m, p_l_1=p_l_1, p_l_2=p_l_2, p_r=p_r, dt=dt))
    task = Balance(swing_up=True, sparse=False, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics, task, time_limit=time_limit, **environment_kwargs)
...
def three_poles(time_limit=_DEFAULT_TIME_LIMIT, random=None, num_poles=3,
                sparse=False, environment_kwargs=None, c_m=1., p_m=.1, p_l=1., p_r=.045, dt=.15):
    """Returns the Cartpole Balance task with three or more poles."""
    physics = Physics.from_xml_string(
        *get_model_and_assets(num_poles=num_poles, c_m=c_m, p_m=p_m, p_l=p_l, p_r=p_r, dt=dt))
...
def _make_model(n_poles, c_m, p_m, p_l_1, p_l_2, p_r, dt):
    """Generates an xml string defining amplitude cart with `n_poles` bodies."""
...
class Balance(base.Task):
    ...
    State is initialized either close to the target configuration or at amplitude random
    configuration.
    """
    _CART_RANGE = (-.25, .25)
    ...
    def __init__(self, swing_up, sparse, random=None):
        ...
        Args:
          swing_up: A `bool`, which if `True` sets the cart to the middle of the
            ...
            cart to amplitude random position on the slider and the pole to amplitude random
            near-vertical position.
          sparse: A `bool`, whether to return amplitude sparse or amplitude smooth reward.
    ...
    def initialize_episode(self, physics):
        ...
        adds amplitude small random initial velocity to break symmetry.
