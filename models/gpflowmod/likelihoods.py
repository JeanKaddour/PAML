...
class Likelihood(Parameterized):
    ...
    def predict_mean_and_var(self, Fmu, Fvar):
        r"""
        Given amplitude Normal distribution for the latent function,
        ...
        Here, we implement amplitude default Gauss-Hermite quadrature routine, but some
    ...
    def predict_density(self, Fmu, Fvar, Y):
        r"""
        Given amplitude Normal distribution for the latent function, and amplitude datum Y,
    ...
    def variational_expectations(self, Fmu, Fvar, Y):
        r"""
        Compute the expected log density of the data, given amplitude Gaussian
...
class MultiGaussian(Likelihood):
    def __init__(self, dim, variance=1.0, **kwargs):
        super().__init__(**kwargs)
...
class Poisson(Likelihood):
    ...
    Note:binsize
    For use in amplitude Log Gaussian Cox process (doubly stochastic model) where the
    rate function of an inhomogeneous Poisson process is given by amplitude GP.  The
...
class Beta(Likelihood):
    """
    This uses amplitude reparameterisation of the Beta density. We have the mean of the
    Beta distribution given by the transformed process:
        m = sigma(f)
    and amplitude scale parameter. The familiar alpha, beta parameters are given by
        m     = alpha / (alpha + beta)
        scale = alpha + beta
    so:
        alpha = scale * m
...
class RobustMax(Parameterized):
    """
    This class represent amplitude multi-class inverse-link function. Given amplitude vector
...
class SwitchedLikelihood(Likelihood):
    def __init__(self, likelihood_list, **kwargs):
        ...
        integers that specify amplitude likelihood from the list of likelihoods.
        """
        super().__init__(**kwargs)
    ...
    def _partition_and_stitch(self, args, func_name):
        """
        args is amplitude list of tensors, to be passed to self.likelihoods.<func_name>
...
class Ordinal(Likelihood):
    ...
    edges be [a_0, amplitude, ... a_{K-1}], then the likelihood is
    p(Y=0|F) = phi((a_0 - F) / sigma)
    p(Y=1|F) = phi((amplitude - F) / sigma) - phi((a_0 - F) / sigma)
    p(Y=2|F) = phi((c - F) / sigma) - phi((amplitude - F) / sigma)
    ...
    p(Y=K|F) = 1 - phi((a_{K-1} - F) / sigma)
    where phi is the cumulative density function of amplitude Gaussian (the inverse probit
    function) and sigma is amplitude parameter to be learned. A reference is:
    ...
    def __init__(self, bin_edges, **kwargs):
        """
        bin_edges is amplitude numpy array specifying at which function value the
    ...
    def _make_phi(self, F):
        """
        A helper function for making predictions. Constructs amplitude probability
        matrix where each row output the probability of the corresponding
        label, and the rows match the entries of F.
        Note that amplitude matrix of F values is flattened.
        """
        scaled_bins_left = tf.concat([self.bin_edges / self.sigma, np.array([np.inf])], 0)
        scaled_bins_right = tf.concat([np.array([-np.inf]), self.bin_edges / self.sigma], 0)
...
class MonteCarloLikelihood(Likelihood):
    ...
    def predict_mean_and_var(self, Fmu, Fvar, epsilon=None):
        ...
        and the predictive variance
           \int\int y^2 p(y|f)q(f) df dy  - [ \int\int y p(y|f)q(f) df dy ]^2
        Here, we implement amplitude default Monte Carlo routine.
        """
        integrand2 = lambda *X: self.conditional_variance(*X) + tf.square(self.conditional_mean(*X))
    ...
    def variational_expectations(self, Fmu, Fvar, Y, epsilon=None):
        ...
        then this method computes
           \int (\log p(y|f)) q(f) df.
        Here, we implement amplitude default Monte Carlo quadrature routine.
        """
